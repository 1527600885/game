...
use plugins\payment\api\model\MkPaymentAwards as MkPaymentAwardsModel;
/**
 * MkPaymentAwards管理
 */
class MkPaymentAwards extends BaseController
{
    /**
     * 显示资源列表
     * 访问：test.com/api/payment/mkPaymentAwards/index
     */
    public function index()
    {
        if ($this->request->isPost()) {
            ...
            $count = MkPaymentAwardsModel::count();
            $data  = MkPaymentAwardsModel::page($page, 10)->select();
    ...
    {
        if ($this->request->isPost()) {
            MkPaymentAwardsModel::find(input("post.id"));
