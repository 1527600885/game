...
class ApiGame extends BaseController
{
	public function __construct(){
		...
		// ];
		// array_key_exists();
		$this->url = 'https://api.cqgame.games/gameboy';	//API 连接
        $this->wtoken="Authorization:eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyaWQiOiI2NGFiNjYxOGQ2NjVmZGZkZGM5MDY5ZTIiLCJhY2NvdW50IjoidW5nX3N3Iiwib3duZXIiOiI2NGFiNjYxOGQ2NjVmZGZkZGM5MDY5ZTIiLCJwYXJlbnQiOiJzZWxmIiwiY3VycmVuY3kiOiJJTlIiLCJqdGkiOiI0MjE1MjU4NDYiLCJpYXQiOjE2ODg5NTQzOTIsImlzcyI6IkN5cHJlc3MiLCJzdWIiOiJTU1Rva2VuIn0.dzeXonpAVy4gP39iH2iaZnjuRNu3F9CWzn5QyMp5VYI";
    ...
    public function gamelink($account,$gamehall,$gamecode,$gameplat,$lang){

        $data = array('account' => $account, 'gamehall' => $gamehall , 'gamecode' => $gamecode,'gameplat'=>$gameplat,'lang'=>$lang);

        $result=$this->send_req($data,'/player/sw/gamelink');
        return $result;
    }


    public function send_req($data,$link){
        $options = array(
            'http' => array(
        ...
        $result = file_get_contents($this->url.$link, false, $context);
    ...
    public function getGames($gamehall){
        ...
        $getBalanceParams = array('gamehall'=>$gamehall);
        $result = $this->get_req('/game/list?gamehall='.$gamehall);
        return $result;
    }


    public function get_req($sendParams){
        //var_dump($sendParams);
        ...
        $result = file_get_contents($this->url.$sendParams, false, $context);
