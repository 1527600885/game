...
class Game extends BaseController
{
    ...
    public function initialize(){
        ...
        $this->gametype = ['slot','FISH','arcade','LIVE','SPORT','lotto','table'];
    }
    ...
    public function gameindex(){
		$data=[];
		$collectionlist=null;
		$gamecode=input("post.gamecode");//游戏分类ID
...
//
        ...
        $slot_game=$this->GameListModel->field('*,if(groom_sort is null,2000000,groom_sort) as groom_sorts')->withoutField('add_time')->where($where)->limit(13)->page(1)->order($hot_order)->select();//查询随机排序
		foreach($slot_game as $k=>$v){
        ...
        //var_dump($this->gamelang);die();

		foreach($table_game as $k=>$v){
			//$gameName=json_decode($v->gameName,true);
			//$v->gameName=croppstring($gameName[$this->gamelang],6);
			$gameImage=json_decode($v->gameImage,true);
			if($this->gamelang=='KO'){
		...
		$data['slot_game']=$slot_game;//computer game
		$data['table_game']=$table_game;//fishing game
        $data['live_game']=$live_game;//fishing game
